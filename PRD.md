...
本文档用于梳理工业动态调度智能体资产管理模块的产品需求，重点说明首期要解决的问题、用户角色、资产范围、核心功能、MVP 范围、验收标准和待确认问题。
...
系统整体设计思路是以“资产统一沉淀、能力标准抽象、运行解耦管理”为核心，将提示词、工具、知识、规则、工作流、权限、版本、运行记录和反馈证据等智能体相关资产集中管理，并通过标准化资产模型和服务接口向外提供能力。系统本身不绑定具体智能体形态、模型服务或 Agent 框架，而是作为智能体无关的资产治理与能力供给平台，支持不同类型、不同平台、不同业务场景的智能体按统一协议接入、检索、调用、校验和沉淀资产。

它的主要作用是帮助企业把分散在人员经验、业务系统、运行日志和智能体任务过程中的能力沉淀为可管理、可复用、可审计、可演进的数字资产，并支撑后续多智能体、多模型、多业务系统场景下的灵活适配和持续优化。
...
资产新增可以引入智能体辅助完成，但智能体的定位是“起草者、结构化助手和校验助手”，不是正式资产的最终发布者。系统应允许智能体基于自然语言描述、任务轨迹、运行日志、SOP、历史案例、人工反馈或外部材料生成候选资产或草稿内容，但正式资产发布仍必须经过人工确认、验证和审批。
...
- 表单根据资产类型动态展示字段。
- AI 可以辅助生成结构化内容、推荐标签、适用范围、风险等级、证据摘要和审批说明，但必须由用户确认。
- AI 可以识别相似资产并提示重复、合并或基于已有资产创建新版本草稿。
- AI 可以发起字段完整性、Schema、范围合法性、命名规范和治理策略预检查。
- 高风险字段修改后必须重新验证。
- 已发布资产不能直接覆盖，编辑时生成新版本草稿。
- 智能体不得绕过人工确认直接发布正式资产，不得直接扩大资产适用范围，不得直接授予工具执行权限。
...
用于从任务全过程、资产引用、工具调用、治理校验和人工反馈多个视角追踪智能体行为，支撑业务人员复盘建议是否可信，也支撑 IT/系统管理员排查工具调用、权限边界和审计问题。

记录内容包括：

- 任务编号、任务类型、触发人、发生时间。
- 任务上下文、任务状态、建议结果、采纳结果和业务执行结果。
...
- 任务轨迹总览：从单个调度任务视角查看 Agent 的完整执行链路、资产引用数、工具调用数、治理校验数、建议结果和人工反馈。
- 资产引用记录：查看 Agent 检索、选用或丢弃了哪些资产，以及引用原因、使用环节和对方案的影响。
- 工具调用记录：查看工具资产或业务系统接口的调用入参、出参摘要、状态、耗时、错误信息、审批关联和幂等键。
- 治理校验记录：查看高风险动作、工具调用、方案生成或写回前的治理校验结果、命中策略、风险等级和下一步动作。
- 人工反馈记录：查看业务人员对 Agent 建议的采纳、调整、驳回、修改内容和反馈原因；首期也可以作为任务轨迹总览中的详情区块呈现。
...
交互逻辑、页面信息架构、页面草图、数据模型、表结构和接口数据结构已统一沉淀到 `SDD.md`。

本需求文档只保留产品目标、用户角色、资产范围、功能需求、MVP 范围、验收标准和待确认问题。
...
| 资产新增辅助 | 基于 Harness 的候选生成、结构化补全、相似资产识别、治理预检查、转草稿 |
...
- 智能体可以辅助新增资产，生成候选资产或正式资产草稿，但发布正式资产必须经过人工确认、验证和审批。
...
- 首期资产新增辅助采用 Hermes、DeerFlow、DeepAgents、LangGraph 或其他 Harness 时，是否优先接入已有内部 Harness。
- Harness 是否需要支持 human-in-the-loop、结构化 JSON 输出、运行轨迹回放、工具权限控制和可观测性，作为准入条件。

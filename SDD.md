...
系统设计采用高内聚、低耦合的原则：资产管理域内部围绕资产生命周期、版本治理、权限策略、验证审批、候选沉淀、使用审计和价值度量形成完整闭环；对外则通过 Asset Gateway、Agent Asset Adapter、Harness Adapter 等稳定接口与具体智能体运行框架、模型服务和业务系统解耦。

在交互、数据和接口设计上，系统不把某一个智能体框架的任务状态、工具定义或流程 DSL 作为核心模型，而是抽象为资产、资产版本、资产权限、资产证据、工具资产、治理策略、使用记录和候选资产等通用对象。不同智能体可以通过适配层接入同一套资产能力，从而实现智能体无关、框架可替换、资产可复用、过程可追溯。
...
- 资产引用记录：以任务为主线，展示 Agent 执行单个任务时检索、选用或丢弃了哪些资产，说明引用原因、使用环节、证据来源以及对任务结果产生的影响。
...
任务轨迹详情页采用“任务基本信息 + 任务节点轨迹 + 节点记录明细”的结构：顶部展示当前任务编号、场景、Agent、触发人、产线、执行时间、任务结果、风险等级和记录数量；中部以流程图展示任务节点轨迹，支持选中节点；底部根据当前节点切换资产引用记录、工具调用记录、治理校验记录和人工反馈记录，用于解释该节点引用了哪些资产、调用了哪些工具、命中了哪些治理策略以及人工如何处理。
...
用于维护权限、范围字典、审批策略、治理策略和审计规则。治理配置不采用单页内横向 Tab，而是在左侧导航中作为 `治理配置` 下的独立子菜单呈现，便于分别承载不同治理对象的配置深度。

建议拆分为以下子菜单：

- 权限配置：配置角色与 Agent 在组织范围内对资产、工具和高风险动作的默认权限边界。
- 范围字典：维护组织、工厂、车间、产线、业务场景、产品族、客户等级等可授权范围编码。
- 审批策略：配置资产发布、候选转正、高风险工具使用、计划写回、跨车间调拨等审批流程、角色、SLA 和升级规则。
- 治理策略：定义 Agent 运行时动作在不同条件下允许、警告、需要审批或阻断的策略规则。
- 审计规则：配置资产使用、工具调用、权限变更、审批流转、治理命中和数据导出的审计记录范围、脱敏、保留周期和告警规则。
